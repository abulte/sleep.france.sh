...
def forward(old_orm, new_orm):
    ...
    return [
        ...
        day.update({day.notes: old_day.notes}).where(old_day.notes.is_null(False)),
    ]
...
def backward(old_orm, new_orm):
    ...
    day = new_orm['day']
    return [
        # Convert datatype of the field day.notes: TEXT -> VARCHAR(255),
        day.update({day.notes: old_day.notes.cast('VARCHAR')}).where(old_day.notes.is_null(False)),
    ]
